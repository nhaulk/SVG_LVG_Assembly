...
# Define paths
ASSEMBLY="/lustre/isaac24/proj/UTK0312/nathaniel.haulk/projects/Dlongi_genomes/LVGandSVG_Juicebox/LVG_juicebox_files_purged/LVG_Male_primary_purged_noec.review_kac_07-10-25.assembly"
PREFIX="LVG_purged_noec"
...
python /lustre/isaac24/proj/UTK0312/nathaniel.haulk/projects/juicebox_assembly_converter.py \
  -a "$ASSEMBLY" \
  -f "$RAWDATA/HiC_yahs_purged/LVG_Male_primary_purged_noec_converted.fasta" \
  -p "$PREFIX" \
  -s
...
cat ${RAWDATA}/HiC_yahs_purged/yahs/LVG_Male_primary_purged_yahsout_noec_scaffolds_final.fa ${RAWDATA}/MT_purged_noec/CM067966.1.fasta > ${PREFIX}.fasta
...
cat ${RAWDATA}/Blob_purged_noec/LVG_Male_primary_purged_noec_blobblurbout.tsv | grep -v "Arthropoda\|record" | cut -f 1 > ${PREFIX}_blob_contaminants.txt
