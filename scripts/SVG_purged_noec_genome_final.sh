...
# Define paths
ASSEMBLY="/lustre/isaac24/proj/UTK0312/nathaniel.haulk/projects/Dlongi_genomes/LVGandSVG_Juicebox/SVG_juicebox_purged/SVG_Male_primary_purged_noec.review_kac_07-10-25.assembly"
PREFIX="SVG_purged_noec"
RAWDATA="/lustre/isaac24/scratch/nhaulk/Dlongi_genomes_USDA/SVG_Onekahakaha_Male"
SOFTWARE="/lustre/isaac24/scratch/nhaulk/software"

# Define output directory
OUTPUTDIR="../SVG_purged_noec_Clean_Genome"
...
python /lustre/isaac24/proj/UTK0312/nathaniel.haulk/projects/juicebox_assembly_converter.py \
  -a "$ASSEMBLY" \
  -f "$RAWDATA/HiC_yahs_p/SVG_Male.p_ctg_noec_converted.fasta" \
  -p "$PREFIX" \
  -s
...
cat ${RAWDATA}/HiC_yahs_purged/yahs/SVG_Male_primary_purged_yahsout_noec_scaffolds_final.fa ${RAWDATA}/MT_purged_noec/CM067966.1.fasta > ${PREFIX}.fasta
...
cat ${RAWDATA}/Blob_purged_noec/SVG_Male_primary_purged_noec_blobblurbout.tsv | grep -v "Arthropoda\|record" | cut -f 1 > ${PREFIX}_blob_contaminants.txt
...
cat $RAWDATA/MT_purged_noec/contigs_ids.txt ${PREFIX}_blob_contaminants.txt ${PREFIX}_debris.txt > ${PREFIX}_trash.txt
